...
-(void)getImagePickerWithAlertController:(UIViewController *)controller alerts:(NSArray *)alerts modelType:(SSImagePickerModelType)modelType pickerBlock:(SSAddImagePicekerBlock)pickerBlock{
    ...
    for(int i=0;i<alerts.count;++i){
        ...
        UIAlertAction *action = [UIAlertAction actionWithTitle:alerts[i] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            
            if([alerts[i] isEqualToString:@"相册"]){
                _wayStyle = SSImagePickerWayGallery;
                [self addImagePickerFromIpc:modelType];
            }
            else if ([alerts[i] isEqualToString:@"图库"]){
                _wayStyle = SSImagePickerWayGallery;
                [self addImagePickerFromIpc:modelType];
            }
            else{
                _wayStyle = SSImagePickerWayCamer;
