    ...
    if(self = [super initWithFrame:frame]){
        ...
        for(int i=0;i<Number;++i){
            for(int j=0;j<Number;++j){
                ...
                btn.bounds = makeRect(0, 0, 15, 15);
                [self addSubview:btn];
                btn.tag = i*Number+j+1;
                btn.userInteractionEnabled = NO;
                btn.selected = NO;
                [btn setBackgroundImage:[UIImage imageNamed:@"dian_huise"] forState:UIControlStateNormal];
                [btn setBackgroundImage:[UIImage imageNamed:@"dian_lvse"] forState:UIControlStateSelected];
                
                if(j==0)btn.left=60;
                else if (j==1)btn.centerX = SCREEN_Width*0.5;
                else btn.right = SCREEN_Width-60;
                
                if(i==0)btn.top=20;
                else if (i==1)btn.centerY = 20+SCREEN_Width*0.5-60;
                else btn.bottom = 20+SCREEN_Width-120;
