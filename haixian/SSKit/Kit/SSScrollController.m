...
//    [self.view addSubview:header];
    ...
    for(int i=0;i<_images.count;++i){
        ...
        if (![imgstr hasPrefix:@"http://"]) {
            imgstr = makeString(URLStr, imgstr);
        }
        ...
        [img sd_setImageWithURL:[NSURL URLWithString:imgstr] placeholderImage:[UIImage imageNamed:ImagePlace1]];
...
-(void)clickOver{
    ...
    if(_delegate && [_delegate respondsToSelector:@selector(SSScrollControllerBtnClick)]){
        [_delegate SSScrollControllerBtnClick];
    }
