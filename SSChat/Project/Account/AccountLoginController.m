...
-(void)loginNetworking:(UIButton *)sender{
    ...
    if([phoneNum isEqualToString:@""]){
        ...
        [self loginIM:sender phoneNum:phoneNum passWorld:passWorld];
    }
}

-(void)loginIM:(UIButton *)sender phoneNum:(NSString *)phoneNum passWorld:(NSString *)passWorld{
   
    [sender addActivityOnBtn];
    
    [[EMClient sharedClient] loginWithUsername:phoneNum password:passWorld completion:^(NSString *aUsername, EMError *aError) {
        [sender closeActivityByBtn:@"登录"];
        
        if (!aError) {
            
            makeUserName(phoneNum);
