...
//开始搜索 体检套餐 特色服务 医疗美容 科室 常见疾病 专家队伍 食物
-(void)startSearch{
    cout(@"开始搜索");
    ...
    if(_searchString.length==0){
        [self showTime:@"请输入联系人"];
        return;
    }
    
    [self.datas addObject:_searchString];
...
-(void)PBSearchFriendCellBtnClick:(NSIndexPath *)indexPath sender:(UIButton *)sender{
    ...
    [[EMClient sharedClient].contactManager addContact:name message:@"我想加您为好友" completion:^(NSString *aUsername, EMError *aError) {
        if (!aError) {
            [self showTime:@"发送成功"];
            [self.invitedUsers addObject:name];
            [self.mTableView reloadData];
        }
        else{
            [self showTime:@"添加失败"];
