...
-(void)setDataDic:(NSDictionary *)dataDic{
    ...
    if(_indexPath.section == 0 && _indexPath.row == 0){
        ...
        NSInteger count = [SSNotificationManager shareSSNotificationManager].unreadCount;
...
//好友申请，群组邀请，聊天室邀请
-(void)setModel:(SSNotificationModel *)model{
    _model = model;

    _mLeftImgView.image = [UIImage imageNamed:@"user_avatar_blue"];
    
    _mTimeLab.text = model.time;
    
    _mTitleLab.text = model.sender;
    [_mTitleLab sizeToFit];
    _mTitleLab.left = _mLeftImgView.right + 10;
    _mTitleLab.top = _mLeftImgView.top ;
    
    _mMessageLab.text = model.message;
    ...
    //未处理
    if(model.status == SSNotificationDefault){
        ...
        //已同意
        if (model.status == SSNotificationAgreed){
            _mBottomLab.text = @"已同意";
        }
        //已拒绝
        else if (model.status == SSNotificationDeclined){
