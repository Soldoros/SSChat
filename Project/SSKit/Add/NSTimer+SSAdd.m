...
//时间(yyyy-MM-dd HH:mm:ss) 转换成时间戳 毫秒为单位
+(long)getStampWithTime:(NSString *)time{
    
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSDate *lastDate = [formatter dateFromString:time];
    long long firstStamp = [lastDate timeIntervalSince1970] * 1000;
    return firstStamp;
    
}

//时间 format格式 转换成时间戳 毫秒为单位
+(NSString *)getStampWithTime:(NSString *)time format:(NSString *)format{
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:format];
    NSDate *lastDate = [formatter dateFromString:time];
    long long firstStamp = [lastDate timeIntervalSince1970] * 1000;
    NSString *string = [NSString stringWithFormat:@"%lld",firstStamp];
    return string;
}
