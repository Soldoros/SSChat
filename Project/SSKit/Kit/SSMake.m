...
//    return [UIFont fontWithName:@"Songti TC" size:_size];
}
...
UIFont *makeFont1(double _size){
    return  [UIFont fontWithName:@"HeadlineA" size:_size];
}

UIFont *makeFont2(double _size){
    return  [UIFont fontWithName:@"blowbrush" size:_size];
}
...
NSMutableAttributedString *setLabColorCenterLine(NSString *string, NSInteger sta, NSInteger len, UIColor *color, CGFloat font){
      ...
      NSDictionary *dic1 = @{NSForegroundColorAttributeName:color};
      NSDictionary *dic2 = @{NSStrikethroughStyleAttributeName: [NSNumber numberWithInteger:NSUnderlineStyleSingle]};
      NSDictionary *dic3 = @{NSFontAttributeName:makeFont(font)};
      
      NSMutableAttributedString *mutableString = [[NSMutableAttributedString alloc]initWithString:string];
      
      [mutableString addAttributes:dic1 range:NSMakeRange(sta,len)];
      [mutableString addAttributes:dic2 range:NSMakeRange(sta,len)];
      [mutableString addAttributes:dic3 range:NSMakeRange(sta,len)];
...
NSMutableAttributedString *getShadowText(NSString *string, UIColor *color){
    ...
    NSMutableAttributedString *mutableString = [[NSMutableAttributedString alloc]initWithString:string];
    [mutableString addAttributes:dic range:NSMakeRange(0,string.length)];
